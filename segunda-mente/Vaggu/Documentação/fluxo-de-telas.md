...
| `/admin` — Cadastro de shopping | Admin | Login; navegação “Cadastrar”; redirecionamento por perfil | `/admin/shoppings/:shoppingId`; `/admin/shoppings` pelo menu | Cadastrar identificação, endereço, operação e foto em três etapas | Valida os campos de cada etapa antes de avançar, preserva os valores ao voltar e abre a ficha após criar; esta rota **não** é uma visão geral administrativa | Implementado |
...
| `/admin/shoppings/:shoppingId` — Visão geral do shopping | Admin | Lista; criação concluída; link direto autorizado | `/admin/shoppings/:shoppingId/estrutura`, `/admin/shoppings/:shoppingId/gerentes`; `/admin/shoppings` após excluir | Consultar contatos e contagens, abrir edição de dados/foto sob demanda e excluir com confirmação | A visão geral exibe resumo de contato e mantém o formulário de três etapas recolhido até a ação “Editar cadastro”; estados sem leitura confirmada continuam indisponíveis | Implementado |
