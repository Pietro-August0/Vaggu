  ...
  return <section className="grid min-w-0 gap-4" aria-label={modo === "admin" ? "Visualização das vagas para configuração" : "Visualização das vagas do estacionamento"}>
    ...
    <div className="rounded-2xl border border-neutral-300 bg-neutral-200 p-3 sm:p-5">
      ...
      {filtradas.length === 0 ? <p className="rounded-xl bg-white p-6 text-center text-neutral-600">Nenhuma vaga corresponde aos filtros.</p> : <>
        ...
        <ul className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,10rem),1fr))] gap-2 sm:hidden">{filtradas.map(vaga => <VagaLista key={vaga.id} vaga={vaga} selecionada={selecionada === vaga.id} aoSelecionar={() => setSelecionada(vaga.id)}/>)}</ul>
        {semPosicao.length > 0 && <div className="mt-3 rounded-xl border border-dashed border-neutral-400 bg-white/80 p-3"><p className="text-sm font-semibold text-neutral-800">Sem posição no mapa</p><ul className="mt-2 hidden grid-cols-[repeat(auto-fit,minmax(min(100%,11rem),1fr))] gap-2 sm:grid">{semPosicao.map(vaga => <VagaLista key={vaga.id} vaga={vaga} selecionada={selecionada === vaga.id} aoSelecionar={() => setSelecionada(vaga.id)}/>)}</ul></div>}
      </>}
...
function VagaLista({ vaga, selecionada, aoSelecionar }: { vaga: VagaComSetor; selecionada: boolean; aoSelecionar: () => void }) {
  return <li className="min-w-0"><button type="button" onClick={aoSelecionar} className={`cartao-clicavel flex h-full w-full min-w-0 flex-col items-start gap-2 rounded-lg border bg-white p-3 text-left ${selecionada ? "border-black ring-4 ring-[#ffe100]/60" : "border-neutral-200"}`}><span className="flex min-w-0 items-center gap-2"><span className={`size-3 shrink-0 rounded-sm ${classesEstado[vaga.estadoAtual]}`} aria-hidden="true"/><strong className="break-all">{vaga.codigo}</strong></span><small className="break-words text-neutral-600">{vaga.setor} · {nomesTipo[vaga.tipo]}</small><Badge variant="secondary">{nomesEstado[vaga.estadoAtual]}</Badge></button></li>
}
